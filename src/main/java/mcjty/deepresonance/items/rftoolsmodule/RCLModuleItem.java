...
public class RCLModuleItem extends GenericDRItem implements IModuleProvider {

    public RCLModuleItem() {
        super("rcl_module");
        setMaxStackSize(1);
    }
    ...
    public void addInformation(ItemStack itemStack, EntityPlayer player, List<String> list, boolean whatIsThis) {
        ...
        list.add(EnumChatFormatting.GREEN + "Uses " + RadiationConfiguration.RCLMODULE_RFPERTICK + " RF/tick");
        ...
        if (!hasTarget) {
            list.add(EnumChatFormatting.YELLOW + "Sneak right-click on a RCL tank to set the");
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (block == TankSetup.tank) {
            ...
            if (world.isRemote) {
                Logging.message(player, "RCL module is set to block '" + name + "'");
            }
        } else {
            ...
            if (world.isRemote) {
                Logging.message(player, "RCL module is cleared");
            }
        }
